...
pseudotty="< /dev/tty6"                                          # bei mir funktioniert /dev/tty6 als Input daher gibt es hier die Möglichkeit eine INPUTOption für wscat anzugeben
                                                                 # ohne die Angabe wird Standard-Input verwendet, dann funktioniert das Script aber nicht im Cron
...
# ich bekomme auf meine Pi das Script in den Hintergrund in dem ich die Eingabe Umleite auf eine "normale" Console Allerdings ändert das nix an der Problematik mit den Waits, es löst nur das Cron-Thema
wscat -c ${intercom} -s webrtc-signaling --slash -P -w ${wscatwait} -x '{"jsonrpc":"2.0","method":"getLastActivities","id":1,"params":[0,100]}' ${pseudotty} > ${tempfile}
